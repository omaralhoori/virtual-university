...
public class teacher {
    ...
    JFrame tf;
    JPanel fram, header, asideBtn, exit, mini, maxi, homeBt, examBt, courseBt, degreeBt;
    JLabel name, number;
    String Name, Number;
    JScrollPane aside;
    teacherLogic logic;
    ...
    public teacher(String Name, String Number) throws IOException {
        this.Name = Name;
        this.Number = Number;
        getTf();
    }

    public JFrame getTf() throws IOException {
        if (tf == null) {
            tf = new JFrame("Öğretmen Sayfası");

            tf.setUndecorated(true);
            tf.setSize(1000, 600);
            tf.setDefaultCloseOperation(EXIT_ON_CLOSE);
            tf.getContentPane().setLayout(new AbsoluteLayout());
            tf.getContentPane().add(getFram(), new AbsoluteConstraints(0, 0, 1000, 30));
            tf.getContentPane().add(getAside(), new AbsoluteConstraints(0, 30, 200, 570));
            tf.getContentPane().add(getHeader(), new AbsoluteConstraints(200, 30, 800, 100));
            tf.getContentPane().add(new homePage(), new AbsoluteConstraints(200, 130, 1000, 570));
            tf.setVisible(true);
    ...
    public teacherLogic getLogic() {
        if (logic == null) {
            logic = new teacherLogic(this);
        }
        return logic;
    }
